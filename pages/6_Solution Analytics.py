...
if 'submission' not in st.session_state:
  st.header("You need a solution first!")
  if st.button("Go to Fleet Optimizer", key = 't1'):
    st.switch_page("pages/2_Fleet Optimizer.py")
  if st.button("Use a sample solution", key = 't2'):
...
if 'submission' in st.session_state:
  ...
  c1.download_button("Download solution", sub.to_csv(index=False), 'solution.csv')
  ...
  if 'original_carbon_emissions' not in st.session_state:
    st.session_state['original_carbon_emissions'] = carbon_emissions_df
